...
class CSFramework_Metabox extends CSFramework_Abstract{
  ...
  public function __construct( $options ){
    ...
    if( ! empty( $this->options ) && defined('ct_metas') ) {
      $this->addAction( ct_metas , 'add_meta_boss' );
  ...
  // add metabox
  public function add_meta_boss( $post_type ) {

    foreach ( $this->options as $value ) {
      if(function_exists('jws_add_meta_boss2')) {
         jws_add_meta_boss2( $value['id'], $value['title'], array( &$this, 'render_meta_box_content' ), $value['post_type'], $value['context'], $value['priority'], $value );
      }  
  ...
  public function render_meta_box_content( $post, $callback ) {
    ...
    echo '<div class="cs-framework cs-metabox-framework">';
      ...
      echo '<div class="cs-body'. $show_all .'">';
        ...
        echo wp_kses_post(( $has_nav ) ? '<div class="cs-nav-background"></div>' : '');
